  ...
  render() {
    ...
    return (
      <Host style={{ display: 'block' }} class={classes}>
        {!loading && !error && <img src={src} alt="" style={styles} class={`${classes}__inner`} />}
        {loading && <div class={`${classes}__placeholder`}>{placeholder}</div>}
        {error && <div class={`${classes}__placeholder`}>{errorText}</div>}
      </Host>
    );
  }
}
