...
export const Slots = setupComposition(`<slot's />`, () => h `
  ${['Single default slot', 'Slot default + <div slot="right">Named Slot</div>', h `<div slot="right">Use h wrapper, if there is reactive stuff inside of me</div>`].map(slot => h `${Story.tplComposition({ slot })}<br>`)}
`);
